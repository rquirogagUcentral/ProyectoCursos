...
public class ServiceLogin {
	...
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})
	//Realiza la actualización de los datos indicados en el formulario de Ingreso a la plataforma
	//Envia datos para actualización de usuario
	//Retorna Respuesta general de consumo, existe o no el usuario.
	public Response ActualizarUsuario(Usuario request)
	{
		...
		try
		{
			 response = usuFact.sendService(request);
			 System.out.print(response.getEstado());
			 if(response.getEstado().equals("OK"))
				{
					return Response.status(Response.Status.OK).entity(response).build();
				}
