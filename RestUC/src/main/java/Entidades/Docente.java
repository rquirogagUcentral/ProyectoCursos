	...
	//Realiza la consulta de la información básica del usuario
		...
		public Usuario getUsuario(Usuario request)
		{
			try
			{
				...
				facultad=pd.consultaFacultad(docente.getIdFacultad());
					Docente doc=new Docente(new Facultad(docente.getIdFacultad(),facultad.getDescFacultad()), docente.getCurriculo());
					request.setDocente(doc);
				return request;
			}
			catch(Exception e)
			{
